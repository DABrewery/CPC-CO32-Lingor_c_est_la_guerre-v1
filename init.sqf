...
/* Spawn au choix des joueurs avec des caisses de matos */
	...
	if (isServer || isDedicated) then {
		...
		_veh = "ACE_medicalSupplyCrate_advanced" createVehicle _pos;
		clearMagazineCargoGlobal _veh;
		clearWeaponCargoGlobal _veh;
		clearItemCargoGlobal _veh;
		clearBackpackCargoGlobal _veh;
...
/* Client global variables */
	...
	opSide = east;
	dispoRsp = false;
	otherRgpDelay = "otherRegroupeDelay" call BIS_fnc_getParamValue;
...
/* ACE Actions def */
	...
	_action = [ 
		...
		[leader grp_prisonners_2, 0, ["ACE_MainActions"], _action] call ace_interact_menu_fnc_addActionToObject;
	//Déclenchement du mode spectateur custom
	_action_custom_spectator = [
		"customSpectate",
		"Spectateur (F10 pour quitter)",
		"",
		{
			[] spawn int_fnc_customSpectate;
		},
		{player inArea trg_SafetyZone}
	] call ace_interact_menu_fnc_createAction;
	[customSpectate, 0, ["ACE_MainActions"], _action_custom_spectator] call ace_interact_menu_fnc_addActionToObject;

/* ACE ACtions def */

/* Init Shinriel's Super powers scripts */
